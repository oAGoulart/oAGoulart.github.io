...
|:---------|:------------|
| [InstallShield3.7z](files/InstallShield3.7z), [SIG](files/InstallShield3.7z.sig); check setup32.exe with [SHA](files/setup32.exe.sha256). | For InstallShield 3, this is the install engine that gets extracted from `_INST32I.EX_` by the 16-bit `setup.exe` (which is the launcher). Put this in the setup folder and run it instead of the `setup.exe`. |
...
### Proven use case: java development kit 1.1.8\_010 setup
